#!/usr/bin/env bash
set -euo pipefail
# Docker-focused dev helper: start postgres containers and run services on the host JVM
# Usage:
#  ./scripts/docker-dev-run.sh start   -> starts postgres containers and services
#  ./scripts/docker-dev-run.sh stop    -> stops services started by this script
#  ./scripts/docker-dev-run.sh status  -> show service status and health endpoints
