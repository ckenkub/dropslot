...
public class AuthController {
    ...
    public ResponseEntity<?> sendVerification(@Valid @RequestBody AuthVerifyDtos.SendVerifyEmailRequest request) {
        authService.sendVerificationEmail(request.email());
        return ResponseEntity.ok().build();
    }

    @PostMapping("/verify")
    public ResponseEntity<?> verifyEmail(@Valid @RequestBody AuthVerifyDtos.VerifyEmailRequest request) {
        authService.verifyEmail(request.email(), request.code());
        return ResponseEntity.ok().build();
    }

    @PostMapping("/password/reset")
    public ResponseEntity<?> requestPasswordReset(@Valid @RequestBody AuthVerifyDtos.PasswordResetRequest request) {
        authService.requestPasswordReset(request.email());
        return ResponseEntity.ok().build();
    }

    @PostMapping("/password/reset/perform")
    public ResponseEntity<?> performPasswordReset(@Valid @RequestBody AuthVerifyDtos.PerformPasswordResetRequest request) {
